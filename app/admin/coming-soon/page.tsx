...
export default function AdminComingSoonPage() {
  const { comingSoonItems, addComingSoonItem, removeComingSoonItem } =
    useShop();
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-bold">New in Store Management</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div>
          <h2 className="text-xl font-bold mb-6">
            Current Items ({comingSoonItems.length})
          </h2>

          <div className="space-y-4">
            {comingSoonItems.length === 0 ? (
              <p className="text-gray-500 italic">No New in Store items yet.</p>
            ) : (
              comingSoonItems.map((item) => (
                <div
                  key={item.id}
                  className="flex gap-4 p-4 bg-white border border-gray-200 rounded-lg shadow-sm items-center"
                >
